...
import { useCreateUsefulInformation } from "@/hooks/use-useful-information";
import type { UsefulInformationRequest } from "@/types/useful.information.types";
import { ArrowLeft, Info, Loader2, Save, Upload, X, FileText } from "lucide-react";

export default function UsefulInformationCreatePage() {
	const navigate = useNavigate();
	const createMutation = useCreateUsefulInformation();
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const request: UsefulInformationRequest = {
				file,
				title: title.trim(),
					...
					<div className="flex-1 flex items-center gap-3">
						...
						<div>
							...
							<p className="text-muted-foreground mt-1.5 text-base">
								Yeni bir faydalı bilgi belgesi ekleyin
							</p>
						</div>
					</div>
				</div>
					...
					<form onSubmit={handleSubmit}>
						<CardHeader className="pb-6 pt-8 px-8">
							...
							<CardDescription className="text-base">
								Yeni bir faydalı bilgi belgesi oluşturmak için aşağıdaki alanları doldurun
							</CardDescription>
						</CardHeader>
