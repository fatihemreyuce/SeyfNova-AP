...
export default function UsefulInformationListPage() {
	...
	const { data, isLoading } = useUsefulInformation(page, size, sort);
	const deleteMutation = useDeleteUsefulInformation();

	const handleDeleteClick = (item: any) => {
		setDeletingId(item.id);
		setDeletingItemName(item.title || `Useful Information #${item.id}`);
		...
		<div className="space-y-6">
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-sm sm:text-base text-muted-foreground mt-1">
							Faydalı bilgi belgelerini ve kaynaklarını yönetin
						</p>
					</div>
				</div>
				<Button
			...
			<div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
				<div className="relative flex-1">
					...
					<Input
						placeholder="Faydalı bilgilerde ara..."
						value={search}
						onChange={(e) => {
			...
			{isLoading ? (
										...
										<TableCell>
											{normalizeImageUrl(item.fileUrl) ? (
												...
												<div className="flex items-center gap-2 text-muted-foreground">
													<FileText className="h-4 w-4 sm:h-5 sm:w-5" />
													<span className="text-xs sm:text-sm">No file</span>
												</div>
											)}
										</TableCell>
										<TableCell className="max-w-[200px] sm:max-w-[250px]">
											<div className="flex items-start gap-2">
												<Info className="h-4 w-4 mt-1 text-primary/70 flex-shrink-0" />
										...
										<TableCell className="text-right">
											<div className="flex items-center justify-end gap-1 sm:gap-2">
												...
												<Button
													variant="ghost"
													size="icon"
													onClick={() => navigate(`/useful-information/edit/${item.id}`)}
													className="h-8 w-8 hover:bg-primary/10"
													title="Düzenle"
												>
				...
				<div className="border rounded-lg bg-card shadow-sm">
					<Empty className="min-h-[400px] border-0">
						...
						<EmptyHeader>
							<EmptyTitle>Faydalı bilgi bulunamadı</EmptyTitle>
							<EmptyDescription>
								{search
									? "Arama kriterlerinize uygun sonuç bulunamadı. Arama terimlerinizi değiştirmeyi deneyin."
									: "Yeni bir faydalı bilgi belgesi oluşturarak başlayın."}
							</EmptyDescription>
						</EmptyHeader>
						{!search && (
							<Button
			...
			<DeleteModal
				...
				title="Faydalı Bilgiyi Sil"
				description="Bu işlem geri alınamaz. Bu faydalı bilgi ve dosyası kalıcı olarak silinecektir."
