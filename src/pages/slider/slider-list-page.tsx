...
import { useSliders, useDeleteSlider } from "@/hooks/use-sliders";
...
export default function SliderListPage() {
	...
	const { data, isLoading } = useSliders(search, page, size, sort);
	const deleteMutation = useDeleteSlider();

	const handleDeleteClick = (item: any) => {
		setDeletingId(item.id);
		setDeletingItemName(item.title || `Slider #${item.id}`);
										...
										<TableCell>
											{normalizeImageUrl(item.imageUrl) ? (
												<img
													...
													alt={item.title}
													className="h-10 w-10 sm:h-12 sm:w-12 object-cover rounded-md border border-border/50"
												/>
											) : (
